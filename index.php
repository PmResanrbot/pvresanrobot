...
############################################################################################
*/
define('API_KEY','276212592:AAFe-odzfA4KzVuUKmNAZMa1HIYxKsR5RrY');
//----######------
...
$token = $textmessage ;
	...
	$ok = $resultb["ok"];
		...
		$source = str_replace("195651268",$from_id,$source);
		save("bots/$un/index.php",$source);	
		file_get_contents("https://api.telegram.org/bot".$token."/setwebhook?url=https://codesupport.ir/pvretor/bots/$un/index.php");
...
elseif ($textmessage == 'خرید ورژن2') {
var_dump(makereq('sendMessage',[
        'chat_id'=>$update->message->chat->id,
        'text'=>"مبلغ `7000`تومان را واریز کنید",
	'parse_mode'=>'MarkDown',
        'reply_markup'=>json_encode([
            'inline_keyboard'=>[
                [
                    ['text'=>"پرداخت و تحویل آنی",'url'=>"https://telegram.me/faktornowbot"]
                ]
            ]
        ])
    ]));
